...
public class Dodecahedron extends Boss {
    ...
    public void performAttack1() {
        if (World.instance().getPlayer() != null) {
            if (attackCount == 175) {
                Projectile fireball = new Projectile(7 * World.instance().getDifficulty(), 5, 640, 200, 500,
    ...
    public void performAttack2() {
        if (World.instance().getPlayer() != null) {
            if (attackCount == 265) {
                Projectile fireball = new Projectile(2 * World.instance().getDifficulty(), 5, 200, 200, 500,
                        "media/Player/fireball.png", 2, 200, 900, 2);
                World.instance().getCurrentLevel().getCurrentScreen().addEntity(fireball);
                World.instance().getCurrentLevel().getObserver().Initialize(World.instance().isLoaded());
            }
            if (attackCount == 220) {
                Projectile fireball = new Projectile(2 * World.instance().getDifficulty(), 5, 1000, 200, 500,
                        "media/Player/fireball.png", 2, 1000, 900, 2);
                World.instance().getCurrentLevel().getCurrentScreen().addEntity(fireball);
                World.instance().getCurrentLevel().getObserver().Initialize(World.instance().isLoaded());
            }
            if (attackCount == 175) {
                Projectile fireball = new Projectile(2 * World.instance().getDifficulty(), 5, 640, 200, 500,
                        "media/Player/fireball.png", 2, 640, 900, 2);
                World.instance().getCurrentLevel().getCurrentScreen().addEntity(fireball);
                World.instance().getCurrentLevel().getObserver().Initialize(World.instance().isLoaded());
            }
        }
